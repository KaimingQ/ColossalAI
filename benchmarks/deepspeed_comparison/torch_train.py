#!/usr/bin/env python3
"""
PyTorch Native Benchmark for LLM Pretraining (Causal LM).
Supports Single GPU, PyTorch DDP, and PyTorch FSDP across 1, 4, and 8 GPUs.
"""
...
def parse_args():
    ...
    p.add_argument("--strategy", type=str, default="fsdp", choices=["single", "ddp", "fsdp"], help="PyTorch strategy")
...
def main():
    ...
    # ---------- Optimizer ----------
    optimizer = torch.optim.AdamW(model.parameters(), lr=args.lr, betas=(0.9, 0.95), eps=1e-8, weight_decay=0.01)
    ...
    dataset = SyntheticDataset(vocab_size, args.seq_len, args.seed, rank)
    gbs_tokens = args.batch_size * world * args.grad_accum * args.seq_len

    if rank == 0:
        print(f"[config] PyTorch Native model={args.model_dir} strategy={args.strategy} params={total_params/1e9:.2f}B "
    ...
    for step in range(1, args.steps + 1):
        ...
        for acc in range(args.grad_accum):
            input_ids, labels = dataset.get_batch(args.batch_size, device)
            try:
        ...
        tflops = compute_tflops(total_params, args.batch_size * world * args.grad_accum, args.seq_len, step_time)
